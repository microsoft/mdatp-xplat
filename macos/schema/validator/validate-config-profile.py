...
parser = argparse.ArgumentParser(
    description='Validate configuration profile against schema'
)
...
def load_plist(path: str) -> list[dict]:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile() as temp_file:
            subprocess.run(
                ["/usr/bin/security", "cms", "-D", "-i", path, "-o", temp_file.name],
                check=False
            )
